...
impl SceneHandler {
    ...
    fn load_overlay(&mut self, file: &str, num: u32, z: i32, visible: bool, assets: &AssetStore) {
        if let Some(bmp) = assets.decode_bitmap_transparent(file, num) {
            let (x, y) = self.member_pos(file, num, assets);
    ...
    fn member_pos(&self, file: &str, num: u32, assets: &AssetStore) -> (i32, i32) {
    ...
    fn load_menu(&mut self, assets: &AssetStore) {
        ...
        // Mulle body: member #125 (static sprite)
        self.load_overlay(f, 125, 10, true, assets);
    ...
    fn load_garage(&mut self, assets: &AssetStore) {
        ...
        if let Some(btn) = MulleButton::new(
            "Tür → Schrottplatz", f, 34, Some(35), Some(Scene::Junkyard), 5, assets
        ) {
        ...
        if let Some(btn) = MulleButton::new(
            "Garagentor → Hof", f, 36, Some(37), Some(Scene::Yard), 5, assets
        ) {
            self.buttons.push(btn);
        }
        ...
        if let Some(btn) = MulleButton::new(
            "Seitentür → Hof", f, 38, Some(39), Some(Scene::Yard), 5, assets
        ) {
            self.buttons.push(btn);
        }
    ...
    fn load_yard(&mut self, assets: &AssetStore) {
        ...
        if let Some(btn) = MulleButton::new(
            "Briefkasten", &f, 42, Some(43), None, 5, assets
        ) {
            self.buttons.push(btn);
        }
    ...
    fn load_world(&mut self, assets: &AssetStore) {
        ...
        // Dashboard overlay at bottom: member #25
        self.load_overlay(&f, 25, 50, true, assets);
    ...
    fn load_destination(&mut self, num: u8, assets: &AssetStore) {
        ...
        match num {
            ...
            92 => {
                ...
                // Figge body (static overlay)
                self.load_overlay(&f, 16, 18, true, assets);
