    ...
    pub fn new(assets: &AssetStore) -> Option<Self> {
        ...
        for &mem in &FUEL_MEMBERS {
            if let Some(bmp) = assets.decode_bitmap_transparent(file, mem) {
                let (px, py) = member_pos(assets, file, mem);
        ...
        let speedo = if let Some(bmp) = assets.decode_bitmap_transparent(file, SPEEDO_MEMBER) {
            let (px, py) = member_pos(assets, file, SPEEDO_MEMBER);
            FrameData { x: px, y: py, width: bmp.width, height: bmp.height, pixels: bmp.pixels }
        } else {
...
fn member_pos(assets: &AssetStore, file: &str, num: u32) -> (i32, i32) {
